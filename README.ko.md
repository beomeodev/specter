...
- **Graphify 코드 그래프** — `/ms.init`이 구축하고 git 훅이 최신으로 유지하는 로컬 tree-sitter 코드 그래프(LLM·API 키 불필요). 에이전트는 코드베이스 재탐색 대신 `graphify query/path/explain`으로 구조 질문에 답함
...
════════════════════════════════════════════════════════════════════
  ...
  │    7. /ms.verify   Codex + Antigravity 짧은 검증
...
| 구현 중 발견된 설계 변경 | Deviations log / `/ms.expand` | 범위 내 편차는 `/ms.implement`가 `specs/<id>/implementation-notes.md`에 append(`/ms.review`가 보고); 요구사항을 대체하는 변경은 `/ms.expand` 경유 |
...
| 명령어 | 역할 |
| --- | --- |
| `/ms.init` | Spec-Kit 설치 + SPECTER 오버레이·훅·백스톱·Graphify 코드 그래프 주입 |
...
- Graphify (`graphifyy`, 버전 핀 — `/ms.init`이 `uv tool install --python 3.12`로 설치; 그래프는 프로젝트별 gitignore된 `graphify-out/`에 위치)
- Claude Code
