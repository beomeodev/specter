...
/ms.checklist
/ms.specify   # docs/prd/feature-map.md의 Feature 섹션을 붙여넣기
```
...
- **Feature Map 게이트**: PRD를 Feature DAG로 쪼갠 뒤, 사양 작성 전에 변환 품질을 검증
...
# 3. Feature Map 검증: PRD coverage, DAG, owner, stub-forward 체크
/ms.checklist

# 아래 단계는 Feature Map의 Feature별로 DAG 순서대로 반복

# 4. 사양 작성: Feature Map의 Feature 섹션을 입력
/ms.specify
...
# 7. 프로젝트 기준 확정, 첫 Feature 계획 후 보통 1회만
/ms.constitution

# 8. TAG 기반 태스크 생성
/ms.tasks

# 9. 구현 전 문서 정합성 검증: spec ↔ plan ↔ tasks
/ms.analyze

# 10. TDD 구현 + TAG 삽입
/ms.implement

# 11. 코드 리뷰 + 실행 게이트: lint/type/test/build/coverage/TAG
/ms.review

# 12. 문서 동기화, 커밋, push, PR
/fin    # CI 포함
/finq   # 빠른 백업용, CI 생략

# 13. 승인된 PR 머지 + 태그 + GitHub Release
/ms.merglease
```

다음 Feature는 `/ms.specify`부터 반복합니다. `/ms.checklist`는 Feature Map 자체가 바뀌었을 때 다시 실행합니다.
...
| `/ms.checklist` | PRD → Feature Map 변환 검증 | `docs/prd/feature-map.checklist.md` |
...
- PRD의 모든 commitment가 Feature owner를 갖는가
...
- Feature Map이 PRD를 복제하지 않고 reference만 유지하는가

`/ms.specify`와 injected `/speckit.specify`는 checklist audit이 없거나, 실패했거나, Feature Map SHA가 달라져 stale 상태면 거부합니다.
