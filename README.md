...
# 2~5단계(1회 PRD 셋업)는 /ms.pre-specter 한 번으로 자동 진행할 수 있습니다.
# verify(PASS/WARN)까지 통과하면 constitution까지 자동으로 이어집니다:
#   /ms.pre-specter @docs/prd/PRD.md [@docs/prd/another.md]
# 각 단계를 직접 실행하려면 아래 순서를 따릅니다.
...
# 아래 단계는 Feature Map의 Feature별로 DAG 순서대로 반복합니다.
# 6~14단계는 /ms.specter 한 번으로 자동 진행할 수 있습니다. clarify만 사람이 답하면 review까지 이어집니다:
...
# 15. 문서 동기화, 조건부 CI, 커밋, push, PR
/ms.fin    # review 후 변경 없으면 CI 생략, 변경 있으면 CI 실행 (--no-ci로 강제 생략)
