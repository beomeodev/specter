...
# 4. Claude & Antigravity가 가 PRD + Codex checklist + Feature Map을 대조해 global gate 생성
/ms.verify
...
# 7. Codex & Antigravity가 이번 Feature checklist를 짧게 검증
/ms.agent-verify
...
# 12. Codex & Antigravity가 구현 전 문서 정합성 검증: spec ↔ plan ↔ tasks
/ms.analyze

# 13. TDD 구현 + TAG 삽입
/ms.implement

# 14. Codex & Antigravity가 코드 리뷰 + 실행 게이트: lint/type/test/build/coverage/TAG
/ms.review
...
| `/ms.review` | 코드 리뷰 + Codex & Antigravity review + 실행 게이트 | `docs/review/` |
| `/ms.up-docs` | Living docs 동기화 | `docs/dev_daily.md`, API docs |
| `/fin` | 문서 동기화 + CI + commit/push/PR (Antigravity 위임) | commit, PR |
| `/finq` | 빠른 commit/push/PR (CI 생략, Antigravity 위임) | commit, PR |
...
구현 후 PR 전에 실행합니다. 기본 실행에서는 Codex & Antigravity가 foreground로 보조 코드 리뷰를 수행하며, 큰 변경에서는 `--background`를 사용할 수 있습니다.
...
- GitHub CLI: `/fin`, `/ms.merglease` 흐름에서 PR/release 자동화 (Antigravity 서브세션에서 구동)
