...
# 4. 프로젝트 기준 확정: 체크된 PRD Feature Map 기준으로 보통 1회만
/ms.constitution

# 아래 단계는 Feature Map의 Feature별로 DAG 순서대로 반복

# 5. 이번 Feature 검증: Source PRDs, PRD references, Commitment Index 반영도 체크
/ms.checklist

# 6. 사양 작성: 체크된 Feature Map 섹션을 입력
/ms.specify
...
`/ms.constitution`은 기본 Constitution을 새로 만드는 명령이 아닙니다. `/ms.checklist --global`로 검증된 PRD Feature Map을 기준으로 Section IX에 **project-wide baseline**을 확정하는 명령입니다.

그래서 현재 명령은 Feature 사이클 안이 아니라 `/ms.checklist --global` 직후에 둡니다. 이유는 간단합니다.
...
/ms.checklist --global    → PRD Feature Map 검증
...
| `/ms.checklist --global` | 전체 PRD set → Feature Map 변환 검증 | `docs/prd/feature-map.checklist.md` |
