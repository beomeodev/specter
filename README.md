...
> [!IMPORTANT]
> **아직은 개인용 템플릿입니다.** 경로·에이전트(Codex/Antigravity)·게이트가 작성자 환경에
> 맞춰져 있어 그대로 쓰는 스타터가 아니라 아이디어 참고용입니다. 다중 사용자 배포는
> [로드맵](#로드맵-멀티-에이전트--공개-배포)에서 진행 중입니다.
>
> **Still a personal template.** Paths, agents, and gates are wired to the author's
> environment — treat it as a reference, not a drop-in starter. Multi-user
...
현재 릴리즈는 `v2.3.0`입니다 — 변경 이력은 [CHANGELOG.md](./CHANGELOG.md)를 보세요.
...
| `/ms.implement` | TDD 구현 + TAG 삽입 (`--to-end`, `--mode tdd\|refactor`, `--task TNNN`) |
...
`/ms.init`은 검증된 Spec-Kit 릴리즈(`v0.11.6`)에 핀됩니다. 업스트림은 pre-1.0이라 통합
