...
- **Constitution**: AI가 따라야 할 프로젝트 기준을 단일 위치에 고정
...
# 2. PRD를 Feature DAG로 분해
/ms.featuremap @docs/prd/PRD.md

# 3. 전체 Feature Map 검증: PRD coverage, Commitment Index, DAG 체크
/ms.checklist --global

# 아래 단계는 Feature Map의 Feature별로 DAG 순서대로 반복

# 4. 이번 Feature 검증: PRD references와 Commitment Index 반영도 체크
/ms.checklist

# 5. 사양 작성: 체크된 Feature Map 섹션을 입력
/ms.specify
...
# 8. 프로젝트 기준 확정, 첫 Feature 계획 후 보통 1회만
/ms.constitution

# 9. TAG 기반 태스크 생성
/ms.tasks

# 10. 구현 전 문서 정합성 검증: spec ↔ plan ↔ tasks
/ms.analyze

# 11. TDD 구현 + TAG 삽입
/ms.implement

# 12. 코드 리뷰 + 실행 게이트: lint/type/test/build/coverage/TAG
/ms.review

# 13. 문서 동기화, 커밋, push, PR
/fin    # CI 포함
/finq   # 빠른 백업용, CI 생략

# 14. 승인된 PR 머지 + 태그 + GitHub Release
/ms.merglease
```

다음 Feature도 `/ms.checklist`부터 반복합니다. `--global`은 Feature Map 전체가 생성되거나 바뀌었을 때만 다시 실행하고, 기본 `/ms.checklist`는 매 Feature 사이클 첫 단계에서 실행합니다.
...
`/ms.constitution`은 기본 Constitution을 새로 만드는 명령이 아닙니다. 첫 번째 Feature의 `spec.md`와 `plan.md`가 나온 뒤, Section IX에 **project-wide baseline**을 확정하는 명령입니다.

그래서 현재 명령은 `/ms.specify` 직전에 두지 않습니다. 이유는 간단합니다.

- `/ms.specify` 전에는 아직 `spec.md`와 `plan.md`가 없습니다.
- Feature Map만으로는 기술 스택, 파일 구조, 검증 전략을 확정하기에 정보가 부족합니다.
- Feature-local 결정을 Section IX에 넣으면 Constitution이 금방 오염됩니다.

정리하면:

```text
/ms.init          → 기본 Constitution 활성화
/ms.constitution  → 첫 plan 이후 Section IX baseline 확정, 보통 1회
```
...
| `/ms.featuremap` | PRD를 Feature DAG로 분해 | `docs/prd/feature-map.md` |
| `/ms.checklist --global` | 전체 PRD → Feature Map 변환 검증 | `docs/prd/feature-map.checklist.md` |
| `/ms.checklist` | 다음 Feature의 PRD 반영도 검증 | `docs/prd/checklists/feature-NNN.checklist.md` |
| `/ms.specify` | Feature 섹션을 GEARS spec으로 변환 | `specs/{id}/spec.md` |
| `/ms.clarify` | 요구사항 명확화 | spec.md 업데이트 |
| `/ms.plan` | 구현 계획 수립 | `specs/{id}/plan.md` |
| `/ms.constitution` | project-wide baseline 확정 | Constitution Section IX, AGENTS.md |
| `/ms.tasks` | TAG 기반 태스크 생성 | `specs/{id}/tasks.md` |
| `/ms.analyze` | 구현 전 문서 정합성 검증 | pass/fail report |
| `/ms.implement` | TDD 구현 + TAG 삽입 | code, tests, TAG blocks |
| `/ms.review` | 코드 리뷰 + 실행 게이트 | `docs/review/`, review state |
...
### `/ms.checklist`: Feature Map 게이트

`/ms.checklist`는 Spec-Kit의 native checklist를 단순 호출하지 않습니다. SPECTER에서 checklist는 spec 작성 전 게이트입니다.

`/ms.checklist --global`은 Feature Map 생성 직후 한 번 실행합니다. 이 단계는 PRD Commitment Index를 기준으로 PRD의 모든 commitment가 정확히 하나의 Feature owner를 갖는지, DAG와 Stub-and-Forward가 성립하는지 검증합니다.

기본 `/ms.checklist`는 매 Feature 사이클의 첫 단계입니다. DAG상 다음 Feature를 고르고, 그 Feature의 PRD references와 PRD Commitment Index rows를 실제 PRD 섹션과 대조합니다.
...
`/ms.specify`와 injected `/speckit.specify`는 global audit 또는 per-Feature audit이 없거나, 실패했거나, Feature Map SHA가 달라져 stale 상태면 거부합니다.

### `/ms.analyze`: 문서 정합성 게이트

구현 전에 `spec.md`, `plan.md`, `tasks.md`가 서로 맞는지 확인합니다.
...
`--fast`나 `--focus`를 사용해도 critical executable gate는 건너뛰지 않습니다.
