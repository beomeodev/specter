...
<p align="center">
    ...
    <a href="#실행-환경과-가드레일"><img src="https://img.shields.io/badge/gates-lint%20·%20type%20·%20test-5a0fc8" alt="gates"></a>
...
`v2.3.0` 하이라이트: **런타임 게이트 · PRD 라이프사이클 확장** — `/ms.pre-specter` PRD-setup
조정자와 `/ms.expand` 증분 PRD 트랙 신설, `/ms.review` Done Criteria Execution 런타임 게이트
(웹 UI는 `webapp-testing`으로 실제 구동), 신규 스킬 `ms-foundation-prd`·`ms-design-baseline`·
`ms-ops-debugging`, 결정론적 게이트 하네스(`specter-gate.sh`·PreToolUse 훅·pre-commit backstop),
`/ms.merglease` semver 자동화, Feature Map SHA에서 Progress Ledger 분리.
...
두 사이클은 각각 한 명령으로 묶여 있습니다. **권장 형태는 bare 호출**입니다 — PRD와
Feature Map은 관례 경로(`docs/prd/*.md`, `docs/prd/feature-map.md`)에서 자동으로 찾습니다.
`@` 첨부는 관례 경로에 있지 않은 파일에만 씁니다: PRD 전체를 첨부하면 실행할 때마다, conductor가
재시작될 때마다 그 내용 전부가 컨텍스트에 다시 주입됩니다(한 프로젝트에서 이 패턴으로 약 105만
토큰이 소모된 사례가 있습니다) — Feature Map과 각 Feature의 `### PRD references`가 이미 필요한
PRD 구간만 정확히 좁혀주므로, 관례 경로에 있는 파일을 굳이 첨부해도 얻는 것이 거의 없습니다.

- **PRD 작성** (0): PRD가 아직 없으면 `ms-foundation-prd` 스킬로 한 번에 한 질문씩 인터뷰하며 PRD를 함께 작성할 수 있습니다 — 산출 형식이 `/ms.featuremap`이 추출하는 commitment·GEARS 수용 기준과 `/ms.expand`가 소비하는 `## PRD Amendment N` 절에 그대로 맞춰져 있습니다.
- **Pre-Feature 사이클** (2~5): `/ms.pre-specter` — `verify`(PASS/WARN)를 통과하면 `constitution`까지 자동으로 이어집니다.
- **Per-Feature 사이클** (6~14): `/ms.specter 006` — `clarify`만 사람이 답하면 `review`까지 자동으로 이어지며, Feature마다 DAG 순서로 반복합니다.
...
메인 사이클(pre-specter → specter 반복) 밖에서 쓰는 트랙들입니다. 어떤 트랙을 탈지는 변경의
성격이 결정합니다:

> **언제 무엇을 쓰나**
> - PRD가 아직 없거나 막연함 (신규 아이디어·레거시 복구·증분 초안) → `/ms.prd`
> - 새 요구사항 없음 (버그·문구·스타일·리팩토링) → `/ms.fix`
> - 될지 안 될지 실험만 해보고 버릴 것 → `spike` 스킬
> - 기존 baseline에 새 요구사항 증분 → `/ms.expand`
> - 제품 전체 재구성 → `/ms.pre-specter`
> - 구현 중 발견된 설계 변경을 spec/plan에 반영 → `/ms.amend`
> - Feature들은 전부 초록불인데 제품 전체가 궁금할 때 → `/ms.audit`

---

#### `/ms.prd` — PRD 공동 작성 (사이클 밖, 게이트 없음)

`/ms.init` 직후, 막연한 아이디어를 파이프라인이 소비할 수 있는 PRD로 바꾸는 대화형
트랙입니다. 어떤 컨덕터에도 속하지 않고 게이트도 없습니다 — PRD 작성은 사람 속도의 사고
작업이기 때문입니다. 핵심은 **작성 전에 unknowns를 드러내는 것**: 블라인드스팟 패스(모르는
줄도 몰랐던 것 교육) → 생존성 게이트("이 골격, 실사용 가능? 아니면 지금 폐기") → 갭 전체를
아키텍처 영향 순으로 총정리한 뒤 **하나씩** `(권장)`안과 함께 해소. 모든 결정은 근거와 함께
`docs/prd/prd-worklog.md`에 누적 기록되고, 흐름 중 낀 사이드 요청은 파킹랏에 담겨 작성 전
전량 소진됩니다(유실 방지 하드 룰).

세 가지 모드: **신규 아이디어** / **레거시 복구**(옛 코드·브랜치·백업에서 의도를 먼저
복원) / **`--amend`**(기존 제품 확장 — `## PRD Amendment N` 절을 작성해 `/ms.expand`로
연결).

```bash
/ms.prd 암호화폐 백테스터를 주식시장용으로 재구성하고 싶어
/ms.prd @docs/notes/idea-dump.md
/ms.prd --amend 업로드 화면에 일괄 처리를 넣고 싶은데 말로 설명이 잘 안 돼
```

완성되면 신규 PRD는 `/ms.pre-specter`로, Amendment는 `/ms.expand`로 넘깁니다.

---

#### `/ms.fix` — 새 요구사항이 없는 수정

버그 수정, 문구 수정, 스타일 조정, 리팩토링은 전체 사양 흐름을 타지 않습니다. TDD·TAG·게이트는
유지하되 spec/clarify/plan/tasks 의례만 생략하는 경량 트랙입니다.
...
#### `spike` 스킬 — 실험만 하고 버리기 (커맨드 아님)

"이게 될까?"를 확인만 하고 싶을 때는 커맨드 없이 자연어로 말하면 됩니다 — "실험해보자",
"프로토타입만 쳐보자" 같은 표현이 `spike` 스킬을 발동시킵니다. 타임박스(T1 ≤15분 / T2
30–60분), **절대 머지 금지**, 산출물은 `docs/spikes/`의 findings note 하나뿐이며, 실험이
새 요구사항으로 판명되면 그 노트가 PRD Amendment의 근거가 되어 `/ms.expand`로 이어집니다.

---

#### `/ms.expand` — 기존 baseline에 새 요구사항 증분 추가

이미 baseline이 있는 제품에 **새 요구사항**을 추가할 때는 전체 재감사(`/ms.pre-specter`) 대신
`/ms.expand`를 씁니다. `/ms.fix`(새 요구사항이 아예 없는 경우)와 `/ms.pre-specter`(제품 전체를
다시 분해·재감사) 사이의 빈틈을 메우는 트랙으로, **그 증분만 디컴포즈해 새 Feature를 만들고
기존 Feature는 다시 감사하지 않습니다.**

**전제조건.** 이미 확립된 baseline이 있어야 합니다 — `docs/prd/feature-map.checklist.md`가
이미 `PASS`/`WARN`이고 Constitution Section IX가 확정된 상태여야 합니다. 결정론적 게이트
체커로 확인할 수 있습니다.

```bash
.specify/scripts/bash/specter-gate.sh   # overall이 MISSING/FAIL이면 먼저 /ms.pre-specter 필요
```

**1단계 — PRD 끝에 Amendment 절 추가 (append-only).** 기존 PRD 파일 **맨 끝**에 최상위 heading으로
새 요구사항을 붙입니다. 반드시 아래 heading 관례를 따라야 하며, 기존 본문은 한 줄도 수정·삭제하면
안 됩니다(순수 추가만 허용).

```markdown
## PRD Amendment 1 — 2026-08-01

<새 요구사항을 PRD의 평소 산문/GEARS 스타일로 작성>
```

> `ms-foundation-prd` 스킬로 이 `## PRD Amendment N` 절을 인터뷰 형식으로 함께 작성할 수 있습니다.
> 산출 형식이 `/ms.expand`가 소비하는 구조에 그대로 맞춰져 있습니다.

**2단계 — 실행.** PRD를 수정한 뒤 실행합니다. 인자 없이 실행하면 변경된 PRD를 자동 감지하고,
`@`로 수정한 PRD를 명시적으로 고정할 수도 있습니다.

```bash
/ms.expand                      # 변경된 PRD 자동 감지
/ms.expand @docs/prd/PRD.md     # 수정한 PRD를 명시적으로 지정
```

**무엇을 하나.** `/ms.expand`는 마지막 감사 시점의 Git Ref 이후 PRD diff를 계산해 (1) append-only
가드(제거·수정 라인이 있으면 즉시 중단)를 건 뒤, (2) `docs/prd/feature-map.md`에 새 Commitment
Index 행과 새 `## Feature NNN:` 섹션·DAG 엣지만 덧붙이고, (3) 증분만 대상으로 Codex delta
체크리스트(`checklist-delta-N.md`)와 (4) 호스트+Antigravity delta 감사(`## Delta Reconciliation N`)를
돌리며, (5) 필요 시에만 Constitution Section IX에 durable 규칙을 **추가**합니다.

**하지 않는 것 (경계).**

- **freeform 입력 거부** — 임시 요구사항 텍스트를 받지 않습니다. 입력은 언제나 PRD 파일에 붙인
  `## PRD Amendment N` 절이어야 합니다(`/ms.featuremap`의 freeform 거부와 동일).
- **전체 재감사 안 함** — `/ms.featuremap`·`/ms.codex-checklist`·`/ms.verify`를 PRD 전체에 다시
  돌리지 않고, 새로 추가된 amendment 텍스트만 읽고 감사합니다.
- **기존 Feature 불변** — 기존 `## Feature NNN:` 섹션이나 기존 Commitment Index 행을 편집·재소유하지
  않습니다. amendment가 기존 commitment의 소유·범위 변경을 요구하면 **중단하고 `/ms.pre-specter`로
  에스컬레이션**합니다.
- **게이트 완화 안 함** — 새 Feature도 `/ms.specify`에서 여전히 checked Feature 섹션, dual-agent
  검증, Constitution Section IX를 그대로 요구합니다.

**3단계 — 인계.** `PASS`/`WARN`이면 새 Feature로 per-Feature 사이클을 시작합니다.
`FAIL`이면 `/ms.specter`로 넘어가지 말고 보고된 문제를 고친 뒤 `/ms.expand`를 다시 실행합니다.
...
#### `/ms.amend` — 구현 중 발견된 설계 변경 반영

구현하다가 spec/plan을 고쳐야 하는 설계 변경이 발견되면 (또는 `/ms.implement`의 Deviations
로그가 요구사항을 대체하는 것으로 판명되면) 이 트랙으로 문서를 따라잡게 합니다. 기존 FR을
직접 고치지 않고 Amendment 섹션으로 대체 기록을 남깁니다.
...
#### `/ms.audit` — 제품 수준 완전성 감사

Feature별 게이트는 각 조각을 자기 spec에 대해 증명할 뿐, **제품 전체**는 구조적으로 보지
못합니다 — 백엔드는 구현됐는데 어떤 화면도 노출하지 않는 기능, 게이트는 전부 초록인데 새
기계에서는 부팅조차 안 되는 제품, Feature 열 개에 걸쳐 조용히 자라난 공격 표면이 그런
사각지대입니다. `/ms.audit`은 이 "전체" 질문들을 주기적·체계적으로 묻는 **자문형** 커맨드로,
아무것도 막지 않고 어떤 컨덕터에도 속하지 않습니다.
...
| 모듈 | 묻는 질문 |
|---|---|
| 노출 감사 | Commitment Index의 각 약속에 사용자가 실제로 **닿을 수 있는가**? (구현됐지만 미노출 탐지) |
| 콜드스타트 E2E | 새 기계에서 클론→README대로 설치→코어 루프가 도는가? (README 자체도 심사 대상) |
| 위협 모델 | 뚫리면 뭘 잃는가(사람이 답) + 실제 config에서 도출한 신뢰 경계·악용 경로 |
| perf/a11y 스모크 | 핫 엔드포인트 p50/p95가 예산 안인가, WCAG 플로어가 실제 CSS에 살아 있는가 |
| 게이트 가치 리뷰 | 최근 N런에서 각 게이트가 실제로 뭘 잡았나 — 일몰 후보·반복 우회를 **보고만** (게이트 변경은 언제나 사람 결정) |
| 블라인드스팟 원장 | 아직 아무도 안 본 곳의 명시적 목록 — 다음 감사의 우선순위 큐 |

**발견 처리.** 모든 발견은 증거(파일·명령 출력·인덱스 행) 인용이 의무이며, P0~P3로 분류되어
기존 트랙으로 라우팅됩니다 — P0/P1 결함 → `/ms.fix`, 새 요구사항으로 판명 → PRD Amendment →
`/ms.expand`, 나머지 → `docs/todo.md`. 산출물은 `docs/audit/AUDIT-<날짜>.md`와 갱신된
`docs/audit/blind-spots.md`입니다.
...
| `/ms.implement` | 선택된 phase/task/TAG scope 구현 + TAG 삽입 (Feature가 UI 표면을 처음 만들면 `ms-design-baseline`으로 디자인 기반 1회 생성) | code, tests, TAG blocks, `docs/design/` |
...
- **미해결 placeholder**: owned commitment나 done criterion에 미해결 placeholder
  (`TBD`, `TODO`, `{{...}}`, `_or_equivalent` 류 토큰)가 남아 있는가 — 어디든 있으면 WARN,
  done criterion 안이면 FAIL (`/ms.specify`·`/ms.clarify` 단계로 넘어가 clarify-time churn을
  일으키기 전에 차단)
...
- **Done Criteria Execution**: Feature Map의 `### Done criteria`와 spec의 acceptance scenario를
  ...
  검증한다. 각 criterion에 맞는 최소 check(실패 테스트 → curl/HTTP 스크립트 → CLI+fixture diff
  → headless 브라우저 → trace 재현)를 골라 세운다. 웹 UI criterion은 `webapp-testing` 스킬
  (Playwright, 서버 lifecycle 관리, 스크린샷/콘솔 로그 캡처, 게이트 실행 시 headless 기본)로
  구동하며, 이때 스크린샷이 `ms-design-baseline`이 만든 디자인 기반이 실제로 렌더되는지도 함께
  확인한다. RUNNABLE FAIL은 코드 게이트 실패와 동일하게 `NOT READY`를 만든다. MANUAL 항목은
