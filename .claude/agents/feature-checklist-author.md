...
6. **Fix rounds**: when your dispatch prompt includes structural-gate
   ...
   check pass.
7. **Verification signals are evidence, not a profile choice.** Compare every
   recorded signal to its cited PRD evidence, report omissions or
   understatement, and write the `Verification Signal Evidence` table. Never
   assign, recommend, or lower a risk profile; the deterministic gate owns
   that computation.
