...
**모든 기능은 `/speckit.checklist`와 동일**하며 다음을 포함합니다.
- 요구 사항 품질 검증(구현 테스트 아님)
- 컨텍스트 기반의 동적 명확화 질문
- 여러 체크리스트 유형(ux.md, api.md, security.md 등)
- "영어를 위한 단위 테스트" 접근 방식
- spec.md 섹션에 대한 추적성
...
체크리스트는 구현이 아닌 **요구 사항 품질**을 테스트합니다.

❌ **잘못됨**(구현 테스트):
- "방문 페이지에 에피소드 카드 3개가 표시되는지 확인"
- "호버 상태가 올바르게 작동하는지 테스트"

✅ **올바름**(요구 사항 테스트):
- "추천 에피소드의 수와 레이아웃이 명시적으로 지정되어 있습니까? [완전성, Spec §FR-001]"
- "모든 대화형 요소에서 호버 상태 요구 사항이 일관되게 정의되어 있습니까? [일관성]"

## 예제 체크리스트 유형

- **`ux.md`**: 시각적 계층, 상호 작용 상태, 접근성 요구 사항
- **`api.md`**: 오류 응답, 속도 제한, 인증 요구 사항
- **`security.md`**: 위협 모델, 데이터 보호, 인증 요구 사항
- **`performance.md`**: 성능 메트릭, 부하 조건, 성능 저하 요구 사항
- **`requirements.md`**: 일반적인 완전성, 명확성, 일관성 검토
