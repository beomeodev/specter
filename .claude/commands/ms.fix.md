...
```
새 요구사항?
 ├ 예  → /ms.featuremap → /ms.checklist --global → /ms.constitution → /ms.checklist → /ms.specify → /ms.clarify → /ms.plan → /ms.tasks → /ms.analyze → /ms.implement → /ms.review → /fin → /ms.merglease
