...
💡 Documentation:
- Workflow documents (spec.md, plan.md, tasks.md) → created by /ms.* commands
- Living Documents (docs/api/[TAG-ID].md) → auto-generated by /ms.implement
- AGENTS.md → created by /ms.constitution

💡 Tip: You can use /speckit.* commands directly for vanilla Spec-Kit workflow
```
...
### Error 2: Constitution Template Missing

**Symptom**: `templates/constitution-template.md` not found

**Message**:

```
❌ Error: Constitution template not found

Expected location: templates/constitution-template.md

This indicates a repository structure issue.
Please check the my-spec repository integrity.
```

**Exit**: Code 1

### Error 3: Write Permission Denied

**Symptom**: Cannot write to `.specify/memory/constitution.md`

**Message**:

```
❌ Error: Cannot write constitution file

Permission denied: .specify/memory/constitution.md

Please check:
1. Directory permissions for .specify/memory/
2. File permissions if constitution.md already exists
...
-   **Bash**: Execute Spec-Kit installation, check directories, create directories, delete files
-   **Read**: Load constitution template
-   **Write**: Create constitution.md (exact copy of template)
