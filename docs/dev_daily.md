...
- 4차 독립 리뷰: agy PASS(결함 0, 3차에 이어 연속). Codex 레인은 약 55분간 보고서를
  내지 못했고 세션 상태 파일이 30분 이상 정지해 UNAVAILABLE로 기록했다(통과로 간주하지
  않음). 린 계약의 "한 레인 불가 시 비-FAIL 결과는 WARN 상한" 규칙에 따라 4차 판정은
  WARN이며, WARN은 진행 가능하다.
- 리뷰 4라운드 누적: 확정 결함 11건 수정(설계 오류 1건 포함), 범위 밖 기존 결함 3건은
  별건으로 분리 기록.

### 발행 요약 (2026-08-03, fix/gate-map-scope)

**TAG**: @CODE:FIX-GATE-SCOPE-001 / @TEST:FIX-GATE-SCOPE-001 (체인 검사 통과)

**커밋 7개**: 범위 축소 도입 → 펜스 인식 → 리뷰 1차 반영 → 의존 DAG 전역화·파서 정렬
→ 숫자 문법 통일·정규 지도 고정 → 리뷰 결과 기록 (+ 선행 포맷 정리 1건 분리)

**변경 규모**: 7파일, +828/-45

**검증**: make ci 전 항목 통과(테스트 173개, 커버리지 90%+), 무작위 지도 3,000형태
패리티 퍼즈 불일치 0, 실제 git 저장소에서 커밋 백스톱 4시나리오 정상, 독립 리뷰 4라운드
(최종 agy PASS / Codex UNAVAILABLE → WARN 상한).
